...
X<invocation> X<method> X<arrow> X<< -> >> X<safe arrow> X<< &&-> >>
...
You should already be familiar with the use of the C<< -> >> and C<< &&-> >>
operators with references.  In fact, since C<$fred> above is a reference to
an object, you could think of the method call as just another form of
dereferencing.
