...
=head2 The Safe Arrow Operator
X<safe arrow> X<safe dereference> X<< &&-> >>

"C<< &&-> >>" is very similar to the Arrow Operator ("C<< -> >>") except
that it behaves differently when the left side is an C<undef> value, in
which case the result of the expression is also C<undef>.  This allows you
to simplify the dereferencing of complex nested objects or data structures
where undef is an expected possibility:
...
The result of the using the "C<< &&-> >>" on hash or array results will
avoid Perl's default autovivifying behavior (see L<perlref>).  For instance,
this:
...
Mixing "C<< -> >>" and "C<< &&-> >>" can be useful with methods too:

    $a&&->method1->method2;

The expression assumes that $a may be C<undef> and so there will be no error
attempting to call the method operator "method1" but "method1", if called
because $a is not undef, is expected to return a valid reference such that
method2 can be called (and if it doesn't then there will be a fatal error).
